      ...
      <div class="single">
        <div class="info-container">
          ...
          <div class="movie-number">
            <p v-if="list.movies.length > 0">
              Number of movies: {{ list.movies.length }}
            </p>
            <p v-if="list.movies.length > 0">
              Average IMDb rating: {{ averageImdbRating[index] }}
            </p>
            <p v-if="list.movies.length > 0">
              Average personal rating: {{ averageRating[index] }}
            </p>
            <p class="no-movies" v-if="list.movies.length == 0">
              User is yet to add a movie to this list.
            </p>
          </div>
        </div>
      </div>
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      props.lists.forEach((list) => {
        ...
        averageImdbRating.value.push(
          (sumImdb.value / list.movies.length).toFixed(2),
        );
      });
    });
